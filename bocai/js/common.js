$(function () {
    ...
    if (h) {
        affId = h;
        LIBS.cookie("affid", affId)
    }
    var f = LIBS.getUrlParam("a");
    if (f) {
        LIBS.cookie("affCode", f)
    } else {
        f = LIBS.cookie("affCode")
    }
    if (f) {
        var e = $("input[name=affKey]").val(f);
        if (e.length > 0) {
            var b = $(".top .login").removeClass("alogin");
            b.children(".afflogin").remove();
            $('<input id="aff-key-login" type="hidden">').val(f).appendTo(b)
        }
    }
